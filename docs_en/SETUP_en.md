...
| `add` / `import` fails with exit 2 (stderr shows `unknown field(s): ...` or an enumeration of subject candidates) | The v1.9.0 fail-closed — the write path is rejecting an unknown top-level key, an out-of-vocabulary subject, or an out-of-set category | stderr states the cause (the key name, the candidates); fix the record accordingly and rerun. This exists so a typo'd key is not silently discarded, and the read paths (`list` / `get` / `orientation`) remain readable as before (→ DESIGN §3.8/§3.12) |
