...
- **Registry (7 tables)**: three fact-data tables — `INDIVIDUALS` (related parties) / `TASKS` (request progress) / `KNOWLEDGE` (accumulated response knowledge, case-DB-like) — plus `ABILITIES` (a catalog of capabilities the secretary can exercise, §3.8), plus the three role-evolution tables `PROFILE` (person understanding = the P axis) / `GOALS` (goals = the A axis) / `STEPS` (reverse-planned steps) (§3.11)
...
Startup orientation is done with **a single shot of the narrowed digest** from the `orientation` subcommand. The old procedure of `list`ing the 7 tables side by side was structurally bound to break once the registry grew.
...
- **Boundedness of the output (why the size is predictable)**: the digest is a **projection**, not full text, and its volume is roughly bounded by
  ...
  ```
  full text of the small tables (individuals/abilities/profile/goals/steps)
    + tasks count × one-line summary
    + active task count × notes_tail(4000)
    + knowledge count × topic_width(120)
    + handoff_latest(3) × handoff_cap(8000)
  ```

  The essence is that **the dominant terms — "total notes length" and "total knowledge content" — have disappeared from the formula**: records may fatten, the output does not. The defaults come from operational measurement (handovers pile up in the last 3,000–4,000 characters of notes / a topic is identifiable at 120 characters); the two handoff values are extrapolations from measurement and therefore provisional, and all of them can be overridden with CLI options
...
> The cycle of digestion (crystallizing handoff → knowledge) and graduation (archive) is **out of scope for this design = the next stage**. Separation alone is the current ceiling, and the promotion trigger (measured orientation output above 100KB) is inscribed as a `cc-defer` in `registry_cli._read_handoff_blocks`.
