...
回収する。判断の根拠は役割分担の明確化——裸率の降下を起こす作動成分は裸箇所の
**名指し分析**であり、機械はその**当たり付けを安くする**装置に過ぎない。handoff の
書き手は毎枠交代する（注意は構造的に逸れる）ため、注意が逸れた枠で気づかせる装置は
handoff 側でも同じに働く。分担（lint＝当たり付け／名指し＝本体）・別々報告・ゼロを
目標にしない前提は不変（経緯は DESIGN §3.13 に記録）。

### Changed

- **`lint-numbers` の対象を納品物＋handoff へ拡大** — コードは無変更（CLI は元より
  ファイルを選ばない）。変わるのは手順と宣言: `ROUTINE_PROMPT.md` の申し送り節に
  「ブロックを書き上げたら `artifacts-sync` の前に掛け、`bare_lines` の行に計器か
  導出元か継承の別を名指してから送る」を追記、`number_lint.py` の `cc-defer` コメントを
  回収、SKILL / README / commands の表記を「納品物・handoff」へ同期
- `DESIGN.md` §3.13 — 節題を「なぜ納品物だけか」から「どこに掛けるか」へ改め、
  拡大の経緯 bullet を追加
...
> **既存ユーザーの移行手順**: v1.15.0 と同じ——(a) 文書は次回の clone / プラグイン更新で
> 届く。(b) 秘書が handoff にも `lint-numbers` を掛けるようになるのは、**ROUTINE_PROMPT の
> body を各自の cloud routine へ再登録した後**である。routine の更新は各ユーザーの操作で
> 行う——本リポは誰の routine も書き換えない。
...
**問題は表記ではなく注意の配分だった。** 裸数値は表記の癖ではない——一度直しても、
書き手の注意が他へ向いた枠で再発する（母体運用の実測で裸率は大きく振れ、降下と再発を
繰り返した）。ゆえにゼロを目標に置かない（残差は漸近消滅しない）。網は「根絶する装置」
ではなく「注意が逸れた枠で気づかせる装置」である。

**納品物側だけに張るのは、誤読コストが非対称だから。** handoff の読み手は次の枠の自分
自身であり、誤読はその場で訂正が利く——注意で足りている側に二重の関門を足すのは加算
バイアスになる。一方、納品物は読み手が principal とその先の第三者で、誤読のコストが
書き手に返ってこない。
...
- `SKILL.md` / `README.md` / `commands/shiori-secretary.md` — Subcommands 表に 1 行。
...
- `STRUCTURE.md` — `domain/` に `number_lint.py`
- 日英パリティ：上記すべて `docs_en/` / `SKILL_en.md` / `README_en.md` を同内容で更新
...
> **既存ユーザーの移行手順**: (a) コードは**次回の clone / プラグイン更新**から有効になる。
> (b) 秘書が実際に送信前チェックへ `lint-numbers` を掛けるようになるのは、**ROUTINE_PROMPT
> の body を各自の cloud routine へ再登録した後**である（リポ内の md を更新しただけでは
> 稼働中の routine の手順は変わらない）。routine の更新は各ユーザーの操作で行う——本リポは
> 誰の routine も書き換えない。
...
限界は機械の隣（docstring）に、人が最後に見る残りは配布前チェックリストに書く。
併せて構成図の `usecases/observability.py` の欠落と空白区切り行を回収した。**コードの挙動は変えていない**。

### Changed

- **`test_distribution_boundary.py` の docstring に網の限界を明記**——追跡ファイルに語が 1 文字でも現れれば赤（CHANGELOG 等で当の語を書くと踏むので婉曲に書く）／検査語は 2 語のみで組織名・絶対パス・母体のドメイン語彙は対象外／`git ls-files` 依存で tarball 展開では動かない
- **SECURITY 日英の配布前チェックリストに「日英の文書対」の項を追加**——SECURITY は `test_security_doc_parity.py` が節数・箇条数・見出しマーカー列を常設検査、**箇条の順序・訳の内容・他の文書対は目視**と分担を明記（検査は形しか見ない）
- **STRUCTURE 日英の構成図を検査の目に合わせた**——`usecases/observability.py` が図に無かった（`/doc-check` が連結行を分解して読むよう改められ、親の完全性検査で露出）。`adapters/` の空白区切り行 `render/ transcribe/ audio/` は検査に読めないので 1 行 1 実体へ展開

### 移行（稼働中の routine への波及）

**不要**——本版は文書とテストの docstring のみ（配布三則(3)の対象外）。CLI・スキーマ・挙動はいずれも 1.11.4 と同一。

## [1.11.4] - 2026-08-25 — 検査の目から外れていた 2 本を構成図に載せる

構成図のスラッシュ連結行は `/doc-check` に「部分列挙」と読まれ、SECURITY §4/§9 の実装 SSoT 2 本が図に無くても検出されなかった。
展開して機械の目に入れる。**コードの挙動は変えていない**。
...
**不要**——本版は文書とテストフィクスチャのみ（配布三則(3)の対象外）。CLI・スキーマ・挙動はいずれも 1.11.3 と同一。

## [1.11.3] - 2026-08-25 — 同じ穴が三度開いたので、目視をやめて検査を置く

英語版 SECURITY が日本語正本から離れる乖離は、1.11.2 の §4/§9 を直した後、その検収でさらに §1/§2 に見つかった——**3 度目**である。
1.11.1 は見出しの数、1.11.2 はマーカー列と、目視の層を一段ずつ下げてきたが、下げた次の層（箇条の数）に同じ穴が空いていた。
1.11.2 の計画が「3 度目で装置化」を条件として置いていたので、ここで目視を機械の網に替える。**コードの挙動は変えていない**。

### Added

- **SECURITY 日英パリティの静的検査 `scripts/tests/infrastructure/test_security_doc_parity.py`**——`docs/SECURITY.md` と `docs_en/SECURITY_en.md` を `## ` 見出しで節に切り、①節数 ②節ごとの箇条（`- ` 始まり）の数 ③見出しの状態マーカー列（✅/⚠️/📋、異体字セレクタを落として比較）の三点が一致することを検査する。失敗時は節名と JA/EN の件数を**全件列挙**する。見るのは**形だけ**で訳文の質や意味の一致は判定しない（機械に決められない領域を持ち込まない）。対象も SECURITY の 1 対に限る——他の日英対に広げるのは、そこで乖離が観測されてからでよい。本体コードを import しない静的検査である

### Fixed

- **`docs_en/SECURITY_en.md` §1 / §2 に正本の箇条が欠けていた**——§1 の「未認可アクセスの記録」（`[security] unauthorized_update_discarded ...` の stderr 1 行、本文は載せない）、§2 の「入力正規化 `normalize_input`」と「適用範囲は外部入力面の全て」（添付の `rendered_text` と音声の `transcript` にも正規化 → フラグ判定の順を適用）の計 3 箇条を、日本語正本の位置に対応させて訳出した（JA は不変）。**§7 の訳語ゆれ**も `population scope` → `audience scope` に統一した（CHANGELOG_en の既存表記に揃えた）

### 移行（稼働中の routine への波及）

**不要**——本版は文書とテストのみで、読み取り経路で発火する検証は増えていない（配布三則(3)の対象外）。CLI・スキーマ・挙動はいずれも 1.11.2 と同一で、**prompt body の再登録も要らない**。
