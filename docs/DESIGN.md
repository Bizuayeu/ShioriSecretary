...
- **タスク id トークン（v1.17.0、命名を読むだけの二つ目の規約）**: `orientation` の `## artifacts` 節は `artifacts/` 配下（`handoff/` を除く）のパスを列挙し、パス中のタスク id トークン（`t0007/` のディレクトリ成分・`_t0013_` のファイル名成分）で群にして、active タスクの群だけファイル名を並べる。これは秘書が自然発生させた命名を**読むだけ**で、ディレクトリ構成もスキーマも与えない——中身は開かず（退行テストで `Path.open` 不在を固定）、トークンの無い legacy は `untagged` として件数だけ載る。なぜ載せるかは §3.12（監査証跡と採択済みの読み口の非対称）。標準化の線は handoff と同じ「置き場と命名」の内側にある
...
- **監査証跡と採択済みの読み口を揃える（`## artifacts`、v1.17.0）**: tasks の `notes` は追記のみで撤回を書かない**監査証跡**である——置き換えられた基準が断定形のまま印もなく残る。一方、採択された結論は成果物にあるが、それは索引の外だった。分けて置いてあっても**起動時の射影に載る側**に読み手は落ちる（母体運用で、notes の古い基準を成果物より先に読んで答え、後から訂正する事故が出た）。処方は「成果物の中身を載せる」ではなく**索引をもう一つ、notes の直後に置く**——active タスクごとの成果物ファイル名だけを並べ、notes から引いた値を外へ出す前に採択済みを `Read` する読み順を配置で作る。中身を載せないのは §3.10（スキーマレス・解釈しない）と有界性の両方から。規律を knowledge に書くだけでは効かない（上流配置の原則）ので body に置いた
- **knowledge の検索口（`knowledge search`、v1.17.0）**: 新しい知見を焼く前の既出照合は、索引（`id | subjects | topic`）では足りない——索引に無い語で書かれた既出は content の中にしか無い。口が無いと秘書は自前の部分文字列走査を全件に書くことになり、件数に比例して結晶化が遅くなる。検索は read-only の純関数（NFKC → casefold の部分文字列一致、AND／OR）で、出力は索引行に限る（本文は `get --key`）——`list` と同じ沈黙失敗を新しい口で作らないため、見出しに母数を開示し、総バイトを stderr に申告し、閾値超は絞り方を添えて警告する。0 件は観測結果であってエラーではない
...
数値に出所（実測なのか見込みなのか継承なのか）が添えられていないと、読み手はどの数を信じてよいか決められない。`lint-numbers` はこの「裸の数値」を機械で拾うが、**拾うのは presence だけ**である。対象は**納品物と handoff の両方の原稿 md**（v1.15.0 は納品物のみ、v1.15.1 で handoff へ拡大——経緯は本節内）。いずれも消極的な設計判断ではなく、母体運用の実測から出た線である。

- **問題は表記ではなく注意の配分（母体運用の実測）**: 裸数値は表記の癖ではない——一度直しても、書き手の注意が他へ向いた枠で再発する（母体運用の実測で裸率は大きく振れ、降下と再発を繰り返した）。ゆえに**ゼロを目標に置かない**（残差は漸近消滅しない）。機械の網は「根絶する装置」ではなく「注意が逸れた枠で気づかせる装置」として設計する
- **導入時なぜ納品物からか（誤読コストの非対称）**: handoff の読み手は次の枠の自分自身であり、誤読はその場で気づいて訂正が利く。一方**納品物の読み手は principal とその先の第三者**で、誤読のコストが書き手に返ってこない。**網はまず誤読コストの非対称な側に張った**
- **handoff への拡大（v1.15.1）**: 当初は適用拡大を `cc-defer`（運用で必要が示されたら再検討）として先送りしていたが、トリガーを待たず前倒しで回収した。判断の根拠は役割分担の明確化——**降下を起こす作動成分は裸箇所の名指し分析であり、機械はその当たり付けを安くする**装置に過ぎない。handoff の書き手は毎枠交代する（注意は構造的に逸れる）ため、注意が逸れた枠で気づかせる装置は handoff 側でも同じに働く。**分担（lint＝当たり付け／名指し＝本体）、納品物側と handoff 側の裸率を別々に報告する規律、ゼロを目標に置かない前提は、拡大後も不変**——lint が緑でも名指し分析の代わりにはならない（緑の危険は下の偽陰性の項）
...
- **なぜ原稿 md 側に掛けるか**: 納品物は md 原稿と html の対で作られる運用実態がある。生 HTML に行走査を掛けると属性値・px・hex 色でほぼ全行が候補化し、**ノイズに慣れて緑を読み飛ばす**状態を自前で作る。タグ剥がしは判定の入口なので実装しない——**HTML しか無い納品物で候補ノイズが出るのは仕様**とし、原稿 md に掛けるのを正規手順とする（handoff はもとより md なのでそのまま掛かる。手順は ROUTINE_PROMPT）
- **校正はデータ 1 箇所で行う**: 計器トークンは実践語彙（同じ行に計器か導出元か継承の別を名指していれば「出所を添えた」と見なす三分類）を語形の部分文字列一致で近似したものである。運用で偽陽性・偽陰性の偏る語が見えたら `GAUGE_TOKENS` の追補で直す——**定義の校正がコード構造に触れない**形にしてあり、配布先が自分の運用語彙へ校正してよい
