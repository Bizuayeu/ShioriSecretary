...
ROWS = [
    _k("K-001", topic="経費精算の基準", content="旧基準ではなく新基準で読む"),
    _k("K-002", topic="台帳の先頭は腐る", content="追記式の台帳は…", subjects=["経理"]),
    _k("K-003", topic="ＬＬＭの器", content="Weights are build artifacts"),
]


def test_search_matches_substring_in_content_not_only_topic():
    hits = search_knowledge(ROWS, ["旧基準"])
...
def test_search_matches_id_and_subjects_too():
    ...
    assert [r["id"] for r in search_knowledge(ROWS, ["経理"])] == ["K-002"]
...
def test_search_all_terms_must_match_by_default():
    assert [r["id"] for r in search_knowledge(ROWS, ["精算", "新基準"])] == ["K-001"]
    assert search_knowledge(ROWS, ["精算", "台帳"]) == []


def test_search_any_term_matches_when_match_all_is_false():
    hits = search_knowledge(ROWS, ["精算", "台帳"], match_all=False)
