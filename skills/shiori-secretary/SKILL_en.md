...
| `orientation [--notes-tail N] [--topic-width N] [--handoff-latest N] [--handoff-cap N] [--knowledge-category CAT]` | **The startup orientation digest** (a read-only projection replacing a side-by-side `list` of the 7 tables). Emits to stdout the role judgment + per-table record counts/byte sizes + the full text of the small tables + one-line task summaries and the notes tail of active tasks (4000 chars by default) + the `id\|topic` index of knowledge (width 120 by default) + the latest handoff blocks (3 by default, each 8000 chars). The output size is bounded independently of notes length. `--knowledge-category` narrows the index by exact category match (the heading carries `N of M`, so what was filtered out stays visible; 0 matches still exits 0 = narrowing is observation, not validation). Dig deeper with an individual `get --key` | 0=OK, 2=missing config |
