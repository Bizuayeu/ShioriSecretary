...
4. Startup orientation = bulk-load the 7 registry tables (individuals/tasks/knowledge/abilities/profile/goals/steps) into context via list + `role-status`, settling today's role (secretary/butler/coach/anego), then judge actionability for free time (autonomous turn) — under a grant, proactively advance one item such as a continuing-task push or an accompaniment nudge for STEPS near their deadline; if nothing qualifies, focus on inbound. Details in ROUTINE_PROMPT Step 5
