...
| 2 | config missing / malformed / **fail-closed on a write path** (an unknown top-level key, an out-of-vocabulary subject, an out-of-set category) | Check env vars. For fail-closed, stderr states the cause (the key name, the candidate list) — read it, fix the input, and rerun |
