...
8. At session end, `Write` the handover to `registry_dir/artifacts/handoff/<UTC datetime>_<session_id>.md` → `artifacts-sync` (the window is the block boundary; do not append long text to a task's notes) → lease release (so the next cron can pick it up)
```
