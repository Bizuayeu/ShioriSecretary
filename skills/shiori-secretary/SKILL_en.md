...
4. Startup orientation = bulk-load the 4 registry tables (individuals/tasks/knowledge/abilities) into context via list, then judge actionability for free time (autonomous turn) — under a grant, proactively push one item such as a continuing task; if nothing qualifies, focus on inbound. Details in ROUTINE_PROMPT Step 5
...
| `wal-append --kind <individuals\|tasks\|knowledge\|abilities\|outbound> (--json \| --json-file)` | Append the intent to the WAL as pending (**before a registration-type reply**, the push-ahead write for the word-deed consistency guarantee). `outbound` is normally encapsulated by `proactive-send`; manual use is discouraged. Only when `registry_sync` is enabled, no-op when disabled | 0=OK, 2=invalid |
