...
- **state persistence**: `offset.json` + `lease.json` are saved in `state_dir`, with heartbeat + TTL lease for concurrency prevention and crash self-healing. **The registry (7 tables: individuals/tasks/knowledge/abilities/profile/goals/steps) lives in `registry_dir`, separated from the volatile state, and when `registry_sync` is enabled it is git-persisted to a fixed branch** (event-driven commit&push + fetch at startup, no force)
...
| `individuals\|tasks\|knowledge\|abilities\|profile\|goals\|steps {list\|get\|add\|remove}` | CRUD on the registry (7 tables: INDIVIDUALS/TASKS/KNOWLEDGE/ABILITIES/PROFILE/GOALS/STEPS). `get`/`remove` use `--key` (uuid/id), `add` uses `--json`/`--json-file`. Validated by value objects. **The `category` of knowledge accepts only the allowed set of 10** (`observation` / `research` / `harness` / `domain-insight` / `analysis` / `design` / `method` / `philosophy` / `business` / `decision`) — both an out-of-set value **and a missing one** exit 2 with a stderr message that enumerates the allowed set (v1.8.0; the implicit `"general"` fallback is gone = you are simply rejected, and the information needed to pick the right word is in the error text). `category` is reserved for the axis of **the type of knowing**; the subject axis (a project, a client, …) is carried by a prefix on `topic` in the form `[subject] body` (e.g. `[accounting] monthly close procedure`) — **a convention, not an enforcement** (neither the code nor the data requires the prefix; when to start and whether to backfill existing topics is the secretary's discretion). The SSoT is Private JSON, the operating entity is SecretaryRole, the entry point is `/shiori-secretary`. When `registry_sync` is enabled, commit&push after add/remove (event-driven) | 0=OK, 2=invalid input |
| `orientation [--notes-tail N] [--topic-width N] [--handoff-latest N] [--handoff-cap N] [--knowledge-category CAT] [--knowledge-latest N]` | **The startup orientation digest** (a read-only projection replacing a side-by-side `list` of the 7 tables). Emits to stdout the role judgment + per-table record counts/byte sizes + the full text of the small tables + one-line task summaries and the notes tail of active tasks (4000 bytes by default) + the `id\|topic` index of knowledge (width 120 bytes by default) + the latest handoff blocks (3 by default, each 8000 bytes). **Widths are counted in UTF-8 bytes** and rounded at character boundaries (count in the same unit as the diversion threshold, DESIGN §3.12). The output size is bounded independently of notes length. `--knowledge-category` narrows the index by exact category match (the heading carries `N of M`, so what was filtered out stays visible; 0 matches still exits 0 = narrowing is observation, not validation). `--knowledge-latest` narrows the index to the N newest entries (unset by default = all; the heading reads `latest N of M, newest last`. What is selected is the newest, but the ordering stays ascending by id = **the last line is the newest**, and the `newest last` in the heading discloses that reading. Combined with category, it narrows first and takes latest second). **The total byte size of the digest is reported to stderr on every run as `orientation digest: N bytes`** (v1.8.0; above 25,600 bytes a warning is appended with the diversion risk and the narrowing options. stdout and the exit code are unchanged). Dig deeper with an individual `get --key` | 0=OK, 2=missing config |
...
| `wal-append --kind <individuals\|tasks\|knowledge\|abilities\|profile\|goals\|steps\|outbound> (--json \| --json-file)` | Append the intent to the WAL as pending (**before a registration-type reply**, the push-ahead write for the word-deed consistency guarantee). kind covers all 7 registry tables + outbound (the choices are derived from `REGISTRY_SPEC` = they track table additions automatically). `outbound` is normally encapsulated by `proactive-send`; manual use is discouraged. Only when `registry_sync` is enabled, no-op when disabled | 0=OK, 2=invalid |
