...
4. 起動時オリエンテーション = `orientation` 一撃（role 判定 + 8表の件数/射影 + handoff 最新ブロック）でコンテキストを立ち上げ、今日の役割（秘書/執事/コーチ/アネゴ）を確定。**8表を並べて `list` しない**——肥大した registry では出力上限を超えてコンテキストに載らないまま exit 0 する（沈黙失敗、DESIGN §3.12）。本文が要る個票だけ `get --key` で引く。**notes から引いた値を外へ出す前に `## artifacts` に挙がった採択済み成果物を見る**（notes は撤回を書かない監査証跡）。続けて自由時間（autonomous turn）の actionability 判断（grant 下なら継続型タスクの能動 push・STEPS 期限近接の伴走ナッジ・未消化 handoff の消化〔`knowledge search` で既出照合 → knowledge へ結晶化 → `handoff-archive` で卒業〕等を1つ、値しなければ inbound 専念）。詳細は ROUTINE_PROMPT Step 5
